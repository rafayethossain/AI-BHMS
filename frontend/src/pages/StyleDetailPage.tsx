...
export default function StyleDetailPage() {
  ...
  const [designSheetId, setDesignSheetId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!id) return;
    merchApi.getDesignSheets({ page_size: '10000' })
      .then((r) => {
        const match = r.data.results.find((s) => s.style_id === id);
        if (match) setDesignSheetId(match.id);
      })
      .catch(() => {});
  }, [id]);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-surface rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => navigate(designSheetId ? `/design-sheets/${designSheetId}` : '/design')}
              ...
              <dl className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                ...
                <div className="bg-surface-alt/40 rounded-lg px-3 py-2"><dt className="text-xs text-muted">Buyer</dt><dd className="text-heading">{style.buyer_name || '—'}</dd></div>
