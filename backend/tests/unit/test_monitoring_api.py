...
from apps.monitoring.models import AuditLog, SystemHealth, Alert
...
class TestHealthCheck:
    def test_health_check_returns_200_when_authenticated(self, mon_client):
        response = mon_client.get("/api/v1/monitoring/health/")
        assert response.status_code == status.HTTP_200_OK

    def test_health_check_returns_list(self, mon_client, mon_tenant):
        response = mon_client.get("/api/v1/monitoring/health/")
        assert response.status_code == status.HTTP_200_OK

    def test_run_health_checks(self, mon_client):
        response = mon_client.post("/api/v1/monitoring/health/run_checks/")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
...
class TestAlertAPI:
    ...
    def test_create_alert(self, mon_client, mon_tenant):
        response = mon_client.post("/api/v1/monitoring/alerts/", {
            "alert_type": "warning",
            "service": "production",
            "title": "Test Alert",
            "message": "This is a test",
            "tenant": str(mon_tenant.id),
        })
...
class TestSystemHealthAPI:
    ...
    def test_retrieve_health_check(self, mon_client, mon_tenant):
        ...
        response = mon_client.get(f"/api/v1/monitoring/health/{health.id}/")
        assert response.status_code == status.HTTP_200_OK
        assert response.data["service"] == "database"

    def test_health_read_only(self, mon_client, mon_tenant):
        response = mon_client.post("/api/v1/monitoring/health/", {
    ...
    def test_filter_health_by_service(self, mon_client, mon_tenant):
        ...
        response = mon_client.get("/api/v1/monitoring/health/?service=database")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_filter_health_by_status(self, mon_client, mon_tenant):
        ...
        response = mon_client.get("/api/v1/monitoring/health/?status=down")
        assert response.status_code == status.HTTP_200_OK
...
class TestMonitoringAuth:
    ...
    def test_authenticated_health_list(self, mon_client):
        response = mon_client.get("/api/v1/monitoring/health/")
        assert response.status_code == status.HTTP_200_OK
